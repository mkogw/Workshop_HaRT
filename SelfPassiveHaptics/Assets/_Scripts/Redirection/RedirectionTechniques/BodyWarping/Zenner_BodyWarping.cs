...
namespace HR_Toolkit
{
    ...
    public class Zenner_BodyWarping : BodyWarping
    {
        ...
        {
            if(useIndex){
                tf_index = indexMarker.GetComponent<Transform>();
            }
            ...
            switch (selectedWarpingMode)
            {
                case WarpingMode.Horizontal:
                    ...
                    if(useIndex)
                    {
                        var horizontalWarp = RotationalWarp(tf_index.position, warpOrigin.position, forward, redirection, redirectionAngleAlpha);
                        virtualHandPos.position = horizontalWarp - (tf_index.position - realHandPos.position);
                    }else
                    {
                ...
                case WarpingMode.Vertical:
                    ...
                    if(useIndex)
                    {
                        var verticalWarp = RotationalWarp(tf_index.position, warpOrigin.position,forward, redirection, redirectionAngleAlpha);
                        virtualHandPos.position = verticalWarp  - (tf_index.position - realHandPos.position);
                    }else
                    {
                ...
                case WarpingMode.GainBased:
                    if(useIndex)
                    {
                        var gainBasedWarp = GainWarp(tf_index.position, warpOrigin.position, redirectionAngleAlpha);
                        virtualHandPos.position = gainBasedWarp  - (tf_index.position - realHandPos.position);
                    }else
                    {
